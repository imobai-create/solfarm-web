...
export default function AreaDetailPage() {
  ...
  async function runAnalysis() {
    setAnalyzing(true)
    try {
      setAnalyzeStep("🛰️ Buscando imagem Sentinel-2...")
      const imgRes = await api.post(`/areas/${id}/satellite/search`)
      const imageId = imgRes.data?.id ?? imgRes.data?.imageId

      setAnalyzeStep("📡 Processando bandas espectrais (NDVI, NDRE, NDWI)...")
      await new Promise(r => setTimeout(r, 2000))

      setAnalyzeStep("🔬 Gerando diagnóstico com IA agrícola...")
      await api.post(`/satellite/process/${imageId}`)

      const diagRes = await api.post(`/areas/${id}/diagnostic`)
      setAnalyzeStep("✅ Diagnóstico completo!")
      await new Promise(r => setTimeout(r, 1000))
      router.push(`/dashboard/diagnostics/${diagRes.data.id}`)
    } catch (err: any) {
      setAnalyzeStep("❌ " + (err?.response?.data?.error ?? "Erro na análise. Tente novamente."))
    ...
    <div className="min-h-screen">
      ...
      <div className="p-8">
          ...
          <Card className="border-green-200 bg-green-50 mb-6">
            <CardContent className="p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-green-800">Análise em andamento</p>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          {[
            { icon: Map, label: "Área total", value: `${area.hectares} ha`, color: "green" },
            { icon: Activity, label: "Diagnósticos", value: diagnostics.length, color: "blue" },
            { icon: latest ? BarChart3 : Satellite, label: "Último score", value: latest ? `${latest.score?.toFixed(1)}/10` : "—", color: "emerald" },
            { icon: Clock, label: "Última análise", value: latest ? formatDate(latest.createdAt) : "Nunca", color: "amber" },
            ...
            {latest ? (
                ...
                <CardContent>
                  ...
                  {latest.ndvi !== null && (
                    <div className="grid grid-cols-3 gap-3 mb-6">
                      {[
                        { label: "NDVI", value: latest.ndvi, icon: Leaf, desc: "Vegetação" },
                        { label: "NDRE", value: latest.ndre, icon: TrendingUp, desc: "Nitrogênio" },
                        { label: "NDWI", value: latest.ndwi, icon: Droplets, desc: "Hídrico" },
                  ...
                  {latest.zones && (
                    <div>
                      <p className="text-sm font-semibold text-gray-700 mb-2">Mapa de Zonas (NDVI)</p>
                      <div className="grid grid-cols-3 gap-1.5">
                        {[...latest.zones].reverse().map((zone: any, i: number) => (
                          ...
                          >
                            ...
                            <span className="opacity-75 text-[10px]">Zona {zone.zone}</span>
                          </div>
                        ))}
                      </div>
              ...
              <Card className="border-dashed border-2 border-gray-200">
                <CardContent className="p-12 text-center">
                  ...
                  <h3 className="font-bold text-gray-900 mb-2">Nenhum diagnóstico ainda</h3>
                  <p className="text-sm text-gray-500 mb-6">
                    Clique em "Nova análise" para processar imagens Sentinel-2 e obter o diagnóstico completo desta área
                  </p>
                  <Button onClick={runAnalysis} className="gap-2">
                    <Satellite className="w-4 h-4" /> Iniciar análise via satélite
                  </Button>
                </CardContent>
              </Card>
            )}
